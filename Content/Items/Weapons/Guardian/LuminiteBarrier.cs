using Microsoft.Xna.Framework;
using Terraria;
using Terraria.DataStructures;
using Terraria.ID;
using Terraria.ModLoader;

using Eternia.Content.Items;

namespace Eternia.Content.Items.Weapons.Guardian
{
    // Luminite tier (post-Moon Lord). Aura inflicts Ichor and lightly regens the
    // wielder and nearby allies. Not subclass-locked. Reuses the TrainingShield
    // texture until real art exists.
    public class LuminiteBarrier : ModItem, IShieldWeapon
    {
        public override string Texture =>
            "ETERNIA/Content/Items/Weapons/Guardian/TrainingShield";

        public int AuraPulseInterval => 14;
        public float AuraRadius => 100f;
        public Color AuraColor => new Color(180, 235, 230);

        public override void SetDefaults()
        {
            Item.width = 44;
            Item.height = 44;
            Item.damage = 128;
            Item.DamageType = DamageClass.Generic;
            Item.useTime = 20;
            Item.useAnimation = 20;
            Item.useStyle = ItemUseStyleID.Shoot;
            Item.channel = true;
            Item.noMelee = true;
            Item.noUseGraphic = false;
            Item.autoReuse = false;
            Item.knockBack = 4f;
            Item.value = Item.sellPrice(gold: 20);
            Item.rare = ItemRarityID.Red;
            Item.UseSound = null;
            Item.shoot = ModContent.ProjectileType
                <Eternia.Content.Projectiles.Guardian.DefensiveAuraProjectile>();
            Item.shootSpeed = 0f;
        }

        public void OnAuraHit(Player owner, NPC target)
        {
            target.AddBuff(BuffID.Ichor, 300);
        }

        // Light ally regen (like the Holy Shield), scaled for endgame.
        public void OnAuraPulse(Player owner)
        {
            if (!Main.rand.NextBool(2))
            {
                return;
            }

            const float healRange = 240f;

            for (int i = 0; i < Main.maxPlayers; i++)
            {
                Player ally = Main.player[i];

                if (!ally.active || ally.dead ||
                    ally.statLife >= ally.statLifeMax2)
                {
                    continue;
                }

                if (i != owner.whoAmI &&
                    (ally.team == 0 || ally.team != owner.team))
                {
                    continue;
                }

                if (Vector2.Distance(owner.Center, ally.Center) > healRange)
                {
                    continue;
                }

                ally.statLife += 3;
                ally.HealEffect(3);
            }
        }

        public override bool Shoot(
            Player player,
            EntitySource_ItemUse_WithAmmo source,
            Vector2 position,
            Vector2 velocity,
            int type,
            int damage,
            float knockback)
        {
            if (player.ownedProjectileCounts[type] == 0)
            {
                Projectile.NewProjectile(
                    source, player.Center, Vector2.Zero,
                    type, damage, knockback, player.whoAmI);
            }

            return false;
        }

        public override void AddRecipes()
        {
            CreateRecipe()
                .AddIngredient(ItemID.LunarBar, 14)
                .AddIngredient(ItemID.FragmentSolar, 6)
                .AddTile(TileID.LunarCraftingStation)
                .Register();
        }
    }
}
