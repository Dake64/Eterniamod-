using System.Collections.Generic;

using Microsoft.Xna.Framework;
using Terraria;
using Terraria.ID;
using Terraria.ModLoader;

using Eternia.Content.Necromancy;
using Eternia.Content.Players;

namespace Eternia.Content.Items.Weapons.Summoner
{
    // Base for every Grimoire. Handles raising the selected dominated creature (left
    // click) and cycling the selection (right click). Concrete grimoires override the
    // ISpecializedGrimoire modifiers to reshape how the whole army behaves.
    public abstract class SpecializedGrimoire : ModItem, ISpecializedGrimoire
    {
        // --- Army modifiers (neutral by default) ---
        public virtual float SummonDamageMult => 1f;
        public virtual float ReserveMult => 1f;
        public virtual float ManaMult => 1f;
        public virtual float MoveSpeedMult => 1f;
        public virtual float SizeMult => 1f;
        public virtual int DefenseDelta => 0;
        public virtual bool Lifesteal => false;
        public virtual int OnHitDebuff => -1;
        public virtual float BossEchoMult => 1f;
        public virtual float CommonMult => 1f;

        // A one-line summary of what this Grimoire does to the army (for the tooltip).
        protected virtual string StyleLine => null;

        public override string Texture =>
            "ETERNIA/Content/Items/Weapons/Summoner/BeginnerNecromancyBook";

        protected void SetGrimoireDefaults(int damage, int rare)
        {
            Item.width = 32;
            Item.height = 32;
            Item.damage = damage;
            Item.DamageType = DamageClass.Magic; // the Necromancer is a Mage subclass
            Item.useTime = 26;
            Item.useAnimation = 26;
            Item.useStyle = ItemUseStyleID.HoldUp;
            Item.noMelee = true;
            Item.mana = 10;
            Item.value = Item.buyPrice(gold: 2);
            Item.rare = rare;
            Item.UseSound = SoundID.Item44;
            Item.shoot = ProjectileID.None;
            Item.autoReuse = false;
        }

        public override bool AltFunctionUse(Player player) => true;

        public override bool CanUseItem(Player player)
        {
            var necro = player.GetModPlayer<NecromancerPlayer>();

            if (!necro.IsActiveNecromancer())
            {
                return false;
            }

            var col = player.GetModPlayer<NecromancerCollectionPlayer>();

            if (col.EligibleEntries().Count == 0)
            {
                return false;
            }

            if (player.altFunctionUse == 2)
            {
                return true; // cycle
            }

            return necro.ReservedLifeFraction < 0.9f;
        }

        public override bool? UseItem(Player player)
        {
            var col = player.GetModPlayer<NecromancerCollectionPlayer>();
            List<GrimoireEntry> entries = col.EligibleEntries();

            if (entries.Count == 0)
            {
                return false;
            }

            if (player.altFunctionUse == 2)
            {
                col.SelectedIndex = (col.SelectedIndex + 1) % entries.Count;

                CombatText.NewText(
                    player.Hitbox,
                    new Color(150, 90, 200),
                    $"Selected: {entries[col.SelectedIndex].DisplayName}");

                return true;
            }

            int idx = col.SelectedIndex % entries.Count;
            GrimoireEntry entry = entries[idx];

            col.EnsureActive(entry.Id);

            if (player.whoAmI == Main.myPlayer)
            {
                Projectile.NewProjectile(
                    player.GetSource_ItemUse(Item),
                    player.Center,
                    Vector2.Zero,
                    entry.MinionType(),
                    player.GetWeaponDamage(Item),
                    Item.knockBack,
                    player.whoAmI);
            }

            return true;
        }

        public override void ModifyTooltips(List<TooltipLine> tooltips)
        {
            tooltips.Add(new TooltipLine(Mod, "GrimoireLeft",
                "Left click: raise the selected undead (reserves life, drains mana)"));
            tooltips.Add(new TooltipLine(Mod, "GrimoireRight",
                "Right click: cycle which dominated creature to raise"));

            if (StyleLine != null)
            {
                tooltips.Add(new TooltipLine(Mod, "GrimoireStyle", StyleLine)
                {
                    OverrideColor = new Color(180, 120, 220)
                });
            }
        }
    }
}
